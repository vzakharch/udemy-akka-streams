...
import org.scalatest.wordspec.{AnyWordSpecLike, AsyncWordSpec, AsyncWordSpecLike}

import scala.concurrent.duration.DurationInt
import scala.concurrent.{Await, ExecutionContext, Future, Promise}
import scala.util.{Failure, Success}

class FlowDocSpec extends TestKit(ActorSystem("akkaQuickstart"))
//  with AnyWordSpecLike {
  ...
  "source is immutable" in {
    ...
    val result1 = source.runWith(Sink.fold(0)(_ + _)) // 55

    val zeroes = source.map(_ => 0) // returns new Source[Int], with `map()` appended
    val result2 = zeroes.runWith(Sink.fold(0)(_ + _)) // 0
    //#source-immutable

    (result1 zip result2) map {case (val1, val2) =>
      assert(val1 == 55)
      assert(val2 == 0)
    }
  ...
  "materialization is unique" in {
    ...
    (sum1 zip sum2) map { case (value1, value2) =>
      assert(value1 == 55)
      assert(value2 == 55)
    }
